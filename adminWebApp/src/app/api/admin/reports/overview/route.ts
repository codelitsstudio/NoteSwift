...
export async function GET(req: NextRequest) {
  try {
    ...
    const courseEnrollmentData = courseEnrollments.map(ce => ({
      name: courseMap[ce._id] || 'Unknown Course',
      value: ce.count,
      fill: '#2563eb' // Blue color for consistency
    }));
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      weeklyActiveUsersData.push({
        ...
        users: Math.max(...activeUsers) // Take the higher count as active users
      });
    }
    ...
    const completedCourses = await CourseEnrollment.countDocuments({
      enrolledAt: { $gte: startDate },
      // Assuming completion is tracked elsewhere, for now use enrollment count
    });
