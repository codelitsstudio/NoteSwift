...
export async function POST(req: NextRequest) {
  try {
    ...
    const reason = body?.reason || 'Violation of terms';
    ...
    (teacher as any).banReason = reason;
    await teacher.save();
    ...
    try {
      await resend.emails.send({
        ...
        html: `<p>Hi ${teacher.firstName || ''}, your account has been banned from NoteSwift. Reason: ${reason}</p>`
      });
