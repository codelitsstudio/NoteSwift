...
export async function POST(req: NextRequest) {
  try {
    ...
    const reason = body?.reason || 'Administrative removal';
    ...
    (teacher as any).removalReason = reason;
    await teacher.save();
    ...
    try {
      await resend.emails.send({
        ...
        html: `<p>Hi ${teacher.firstName || ''}, your account has been removed from NoteSwift. Reason: ${reason}</p>`
      });
