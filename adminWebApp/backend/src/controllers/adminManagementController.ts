...
export const inviteAdmin = async (req: Request, res: Response) => {
  try {
    ...
    if (!canInviteAdmins(admin.role)) {
      return res.status(403).json({ error: 'Insufficient permissions to invite admins' });
    }
...
export const removeAdmin = async (req: Request, res: Response) => {
  try {
    ...
    if (adminToRemove.role === 'system_admin') {
      return res.status(400).json({ error: 'Cannot remove system admin' });
    }
...
export const setSuperAdmin = async (req: Request, res: Response) => {
  try {
    ...
    if (!canSetSuperAdmin(admin.role)) {
      return res.status(403).json({ error: 'Only system admin can set super admin' });
    }
...
export const demoteSuperAdmin = async (req: Request, res: Response) => {
  try {
    const admin = (req as any).admin;

    // Only system admin can demote super admins
    if (!canDemoteFromSuperAdmin(admin.role)) {
      return res.status(403).json({ error: 'Only system admin can demote super admins' });
    }
    ...
    if (adminToDemote.role === 'system_admin') {
      return res.status(400).json({ error: 'Cannot demote system admin' });
    }

    // Can only demote super admins
    if (adminToDemote.role !== 'super_admin') {
      return res.status(400).json({ error: 'Can only demote super admins' });
    }
