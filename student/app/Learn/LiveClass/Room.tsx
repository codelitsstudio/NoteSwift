...
export default function LiveClassRoom() {
  ...
  const { id, title, teacher, subject, audioEnabled: initialAudio, videoEnabled: initialVideo } = useLocalSearchParams();
  
  const [audioEnabled, setAudioEnabled] = useState(initialAudio === 'true');
  const [videoEnabled, setVideoEnabled] = useState(initialVideo === 'true');
          ...
          <View className="flex-1 relative">
            ...
            <View className="absolute top-4 right-4 w-32 h-40 bg-zinc-950 rounded-2xl border-2 border-zinc-800 items-center justify-center shadow-2xl overflow-hidden">
              <View className="flex-1 items-center justify-center bg-zinc-950 w-full">
                {videoEnabled ? (
                  <View className="items-center">
                    <View className="w-14 h-14 bg-gradient-to-br from-purple-500 to-purple-600 rounded-full items-center justify-center mb-2">
                      <MaterialIcons name="person" size={28} color="#FFFFFF" />
                    </View>
                    <Text className="text-white text-sm font-semibold">You</Text>
                  </View>
                ) : (
                  <View className="items-center">
            ...
            <View className="flex-row items-center justify-around">
              ...
              <View className="items-center">
                <TouchableOpacity
                  ...
                  className={`w-16 h-16 rounded-full items-center justify-center shadow-lg ${
                    videoEnabled ? 'bg-zinc-900 border border-zinc-800' : 'bg-red-500'
                  }`}
                  activeOpacity={0.85}
                >
                  <MaterialIcons 
