...
const OngoingCourseCard = ({ item, enrollment }: { item: any, enrollment?: any }) => {
  ...
  // Use correct chapter parameter for navigation
  let chapterParam = 'chapter-1'; // Default to chapter-1 for featured course
  if (item.title === 'Learn How To Actually Study Before It\'s Too Late' || item.isFeatured) {
    chapterParam = 'chapter-1'; // Featured course maps to chapter-1
  } else {
    // For other courses, try to find the correct chapter key
    chapterParam = item.chapterKey || item.chapter || 'chapter-1';
  }

  // Get average progress across all modules/chapters for this course
  let progress = 0;
  console.log('Enrollment moduleProgress:', enrollment?.moduleProgress);
  if (enrollment?.moduleProgress && Array.isArray(enrollment.moduleProgress)) {
    const moduleProgressArr = enrollment.moduleProgress.map((m: any) => m.progress);
    console.log('Module progress array:', moduleProgressArr);
    if (moduleProgressArr.length > 0) {
      progress = Math.round(moduleProgressArr.reduce((a: number, b: number) => a + b, 0) / moduleProgressArr.length);
    }
  } else {
